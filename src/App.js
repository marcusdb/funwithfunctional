import React from 'react';
import {render} from 'react-dom'
import SlideRenderer from './SlideRenderer'
import Example3 from './Example3'
import Example4 from './Example4'
import Example5 from './Example5'
import Example6 from './Example6'
import Example7 from './Example7'
import Example9 from './Example9'
import Example10 from './Example10'
import Example11 from './Example11'
import Example12 from './Example12'

render(<Example3/>, document.getElementById('example3'))
render(<Example4/>, document.getElementById('example4'))
render(<Example5/>, document.getElementById('example5'))
render(<Example6/>, document.getElementById('example6'))
render(<Example7/>, document.getElementById('example7'))
render(<Example9/>, document.getElementById('example9'))
render(<Example10/>, document.getElementById('example10'))
render(<Example11/>, document.getElementById('example11'))
render(<Example12/>, document.getElementById('example12'))
