...
class Example10 extends React.Component {
  ...
  componentDidMount() {
    let input$ = Rx.Observable.fromEvent(document.getElementById('example10_input1'), 'keyup')
  ...
  render() {
    return (
      <div>
        ...
        <input id='example10_input1' style={{
          width: '150px',
          height: '30px',
          fontSize: '24px'
        }}/>
        ...
        <pre><code is className="javascript hljs" data-trim contenteditable>
          {`
            ...
            input$.switchMap(val=>remoteCall(val)).subscribe(this.updateResult)
`}
        </code></pre>
