...
class Example5 extends React.Component {
  ...
  componentDidMount() {
    let plus$ = fromEvent(
      document.getElementById("example5_btn1"),
      "click"
    ).pipe(
      map(() => 1),
    ...
    let minus$ = fromEvent(
      document.getElementById("example5_btn2"),
      "click"
    ).pipe(
      map(() => -1),
      tap(value => this.draw(value, "stream2"))
    );
    minus$.pipe(merge(plus$)).subscribe(value => this.draw(value, "stream3"));
  }
      ...
      <div>
        ...
        <pre>
          <code is className="javascript hljs" data-trim contenteditable>
            {`
    let plus$ = fromEvent(document.getElementById("example5_btn1"),"click").pipe(
      map(() => 1),
      tap(value => this.draw(value, "stream1"))
    );
    let minus$ = fromEvent(document.getElementById("example5_btn2"),"click").pipe(
      map(() => -1),
      tap(value => this.draw(value, "stream2"))
    );
    minus$.pipe(merge(plus$)).subscribe(value => this.draw(value, "stream3"));
